  ...
  if ((firstName, lastName, email, password, username)) {
    try {
      ...
      if (checkMail.length !== 0) {
        ...
        const ethWallet = await generateWallet(Currency.ETH, true);
        const ethVirtual = await vitualCurrency(ethWallet, "ETH");
        const bchWallet = await generateWallet(Currency.BCH, true);
        const bchVirtual = await vitualCurrency(bchWallet, "BCH");
        const dogeWallet = await generateWallet(Currency.DOGE, true);
